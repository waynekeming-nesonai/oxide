    ...
    int __fb_ioctl(unsigned long request, char* ptr){
        switch(request){
            ...
            case MXCFB_SEND_UPDATE:{
                return __fb_send_update(reinterpret_cast<mxcfb_update_data*>(ptr));
            }
            case MXCFB_WAIT_FOR_UPDATE_COMPLETE:{
                return __fb_wait(reinterpret_cast<mxcfb_update_marker_data*>(ptr));
            }
            case FBIOGET_VSCREENINFO:{
                ...
                return __fb_get_vscreeninfo(reinterpret_cast<fb_var_screeninfo*>(ptr));
            }
